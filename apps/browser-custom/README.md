...
- [脚本式任务系统架构](../../docs/script-task-system-architecture.md)：任务程序、Task SDK、
  Task Runner、x-actions-playwright 与 browser-custom 的职责和运行流程。
- [Task Runner 详细处理流程](../../docs/task-runner-processing-design.md)：TaskRun 领取、账户锁、
  浏览器环境、TaskContext、取消、错误映射和资源清理。
- [管理后台功能与组织设计](../../docs/admin-console-design.md)：后台导航、账户、任务、运行记录、
  AI、设置、API 与状态模型。
- [可视化工作流引擎设计](../../docs/visual-workflow-engine-design.md)：已冻结的未来设计，当前不实现。
...
- 支持单个或勾选批量打开、关闭，以及单个重启、状态和进程数量统计。
...
- 代理、出口 IP、时区、语言和 WebRTC 在每次启动前通过 CloakBrowser 官方 GeoIP 链路统一解析一次；启动时不信任历史出口 IP，也不会重复请求。
...
可选的端口、配置目录和 CloakBrowser 二进制路径可以写入项目根目录的 `.env`；程序会在
创建配置存储和读取监听端口之前加载它。已经由 shell 导出的同名环境变量优先级更高。
...
推荐保持“自动匹配出口 IP”：程序在每次启动前通过 CloakBrowser 官方 GeoIP 链路获取一次
当前出口 IP，并用同一次结果统一设置时区、语言和 WebRTC IP；随后以 `geoip=False` 启动
Chromium，避免第二次网络身份解析。轮换代理不会继续使用上一次保存的出口 IP。
