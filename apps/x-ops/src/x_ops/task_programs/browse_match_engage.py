...
SPEC = ProgramSpec(
    name="browse_match_engage",
    version="1.0.0",
    title="浏览、匹配并互动",
    description="完整控制浏览、匹配、点赞与固定或 AI 回复的顺序。",
)
        ...
        for post in posts:
            ...
            if params.like and actions_completed < params.max_engagements:
                ...
                if result_status(result) == "success":
                    liked += 1
                    actions_completed += 1
            if params.reply_mode != "none" and actions_completed < params.max_engagements:
                await context.cancellation.raise_if_cancelled()
                ...
                if result_status(result) == "success":
                    replied += 1
                    actions_completed += 1
        return actions_completed >= params.max_engagements
